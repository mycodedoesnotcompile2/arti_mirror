...
mod test {
    ...
    fn builder_test() {
        ...
        assert_eq!(
            config.protocols,
            vec!["obfs4".parse().unwrap(), "snowflake".parse().unwrap()]
        );
        assert_eq!(
            config.path,
            Some(CfgPath::new("/usr/bin/obfs4proxy".into()))
        );
        assert_eq!(
            config.arguments,
            vec!["--log-min-severity=info".to_string()]
        );
        assert_eq!(config.run_on_startup, true);
        drop(config);
        let config: TransportConfig = TransportConfigBuilder::default()
            .protocols(vec!["obfs4".parse().unwrap(), "snowflake".parse().unwrap()])
        ...
        assert_eq!(config.proxy_addr, Some("127.0.0.1:9050".parse().unwrap()));
