...
use tor_rtcompat::system_time_now;
use tracing::debug;
...
impl BootstrapStatus {
    ...
    pub fn as_frac(&self) -> f32 {
        ...
        self.conn_status.frac() * 0.15 + self.dir_status.frac_at(system_time_now()) * 0.85
    }
    ...
    pub fn blocked(&self) -> Option<Blockage> {
        ...
        } else if let Some(b) = self.dir_status.blockage(system_time_now()) {
            let message = b.to_string().into();
            let kind = b.into();
            Some(Blockage { kind, message })
        } else {
